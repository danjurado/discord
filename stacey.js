...
const token = 'NTg2NTA4NDczMDQzNTE3NDQw.XPpEDA.Wn48vrPAGK0BbHSHg2xNBKwVNQI';
bot.on('ready', () => {
		...
		if((cur_timestamp == prev_timestamp) && (side == `Buy`)){
			...
			//console.log(`XBTUSD ${timestamp} ${total} BOUGHT for ${price_comma}`);         
					...
					if( ( total > 99999) ){
						...
						sendMessage(`${bitmex} ETHUSD **${total_comma}** contracts ${up} for **${price}** @ ${prev_timestamp}	`);
					}
					if( total > 499999 ){
						total_comma = total.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
						sendMessageHIC(`${bitmex} ETHUSD **${total_comma}** contracts ${up} for **${price}** @ ${prev_timestamp}	`);
					}		
					total=0;
		}			
		if((cur_timestamp.substr(6,7) == "15" || cur_timestamp.substr(6,7) == "30" || cur_timestamp.substr(6,7) == "45") && changed==0){
			bot.user.setActivity(`METH ${price_comma}`, { type: 'playing' });
			changed = 1;
